...
- **Touchpoints:** `src/lib/utils/progression.ts:720`, `:955-961`, `:1057-1065`; `src/components/features/WorkoutSetsList.tsx:1415-1635`.

### Progression spec divergences D3–D4: undecided asymmetries in timed/distance progression
- **What:** Two places where `docs/specs/smart-incrementation.md` could not state intent because the code does not reveal it. **D3 (rule SI-17):** `canDeload` covers `weight|smart|reps` only (`progression.ts:776`), so timed and distance work can never deload — and the adjacent comment says "not manual, not time", omitting `distance`, so it is unclear whether the exclusion was decided or inherited. **D4 (rules SI-29, SI-30):** `time` and `distance` additionally require the *most recent* session to meet target (`:1008`, `:1031`) on top of the consensus gate; weight modes have no such requirement.
...
- **Touchpoints:** `src/lib/utils/progression.ts:776-790`, `:1000-1040`; `docs/specs/smart-incrementation.md` (D3, D4).
...
- **Why deferred:** Wants the engine to see the cap, which the history query does not currently select.
- **Unblocked by:** Phase 5 of the plan. Decision `D-1` settles the rule and **supersedes the fallback proposed here**: a set clears when it met its target and, *only where a cap is prescribed*, logged RIR is at least the cap. There is no fallback ladder for uncapped exercises; they clear on the target alone.
- **Touchpoints:** `src/lib/utils/progression.ts` (`metTargetReps`, `buildSuggestion` consensus block), `src/db/schema/programs.ts:118`, `src/lib/actions/workout-sets.ts` (`getProgressiveSuggestions` — `targetRir` is not currently selected), `docs/specs/smart-incrementation.md` (SI-10).
...
- **What:** `program_sets` carries only `target_reps` — there is no `rep_range_min` / `rep_range_max`. The standard scheme (work 8→12, then add load and reset to 8) therefore has nowhere to live. `reps` mode adds the increment to the target with no ceiling (`src/lib/utils/progression.ts:988`) and never converts banked reps back into load, so a target climbs 8 → 10 → 12 → 14 indefinitely. `smart` mode's `adjustedRepsForWeight` is a partial stand-in but only ever lowers, only in that mode, and only on a 2-12 rep set at RPE ≥ 7. Note SI-11's rationale calls the current scheme "double progression"; it is not — it is consensus at a fixed load. That wording should be corrected whether or not the feature is built.
...
- **Touchpoints:** `src/lib/utils/progression.ts` (1048-1066), `src/types/workout.ts` (`SetSuggestion.reason`), `src/components/features/WorkoutSetsList.tsx` (chip switch), `docs/specs/smart-incrementation.md` (SI-21, SI-33).
