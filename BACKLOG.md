...
- **Why deferred:** Adding `pnpm.onlyBuiltDependencies` to `package.json` decides which packages may run arbitrary code at install time, and it changes what actually gets built in the image. That is a supply-chain decision, not a build fix, and it wants to land on its own rather than folded into an unrelated release. Locally, `pnpm` commands need `--config.verify-deps-before-run=false` until it is resolved, and the flag has to come *before* the script name or pnpm forwards it to the script instead of reading it. The pre-push hook runs a bare `pnpm verify` and cannot take a flag at all, so a push needs `PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN=false` in its environment. The `npm_config_` spelling of that variable does not work.
