...
### Un-logging a set logged through the miss sheet does nothing
- **What:** Long-press a set, enter a short rep count, tap "Log set" — that all works, and the row correctly reads "8 × 80kg · 6 done". Then tap the toggle to un-log it and **nothing happens**: the set stays ticked and its rest timer keeps counting down, so `toggleSet`'s un-log branch never runs at all. Caught by `e2e/miss-sheet.spec.ts`, which phase 2 wrote and shipped without ever running — the whole suite ran for the first time on 2026-08-29 (7 of 10 passed). Reproduced on `cd3e8f1`, so it predates the progression rebuild entirely; it is not a regression from any of its phases. Un-logging a set logged by an ordinary short tap works, which is why the smoke pass did not see it.
- **Why deferred:** It is phase 2's territory, not phase 6's, and the cause is not obviously one line — the two candidates are the click not reaching the toggle at all, and `activeCompletedSets` not containing the set so the tap re-runs the *log* branch instead. Folding a fix into the phase that happened to run the test would bury a bug worth its own change.
- **Unblocked by:** Nothing. Instrument which branch of `toggleSet` runs on that second tap; the answer picks the fix. Note the spec is the reproduction — run it rather than reasoning about it.
- **Touchpoints:** `src/components/features/WorkoutSetsList.tsx` (`saveMissSheet`, `toggleSet`), `src/contexts/workout-session-context.tsx` (`completedSetIds`, `removeCompletedSet`), `e2e/miss-sheet.spec.ts`.
