...
### The same exercise twice in one program silently overwrites logged sets
- **What:** Nothing prevents a program holding two `program_exercises` slots for the same exercise (heavy bench then a back-off bench is a normal thing to program), but `workout_sets` is uniquely indexed on `(session_id, exercise_id, set_number)` and `logWorkoutSet` resolves conflicts with `onConflictDoUpdate` (`src/lib/actions/workout-sets.ts:222-247`). Logging set 1 of the second slot therefore **overwrites set 1 of the first slot** — its weight, reps, RIR and notes are replaced by the back-off's, with no error and nothing in the UI to indicate it happened. The `onConflictDoUpdate` is right for what it was added for (re-logging a set must overwrite, and offline replays must be idempotent); the *identity* is what is wrong. A logged set is keyed by exercise and position when it should be keyed by the plan slot it came from.
- **Why deferred:** Found while hardening [`docs/progression-revamp-plan.md`](docs/progression-revamp-plan.md) (prerequisite `P-2`), not by a user report, so the real-world frequency is unknown — it needs a duplicated exercise in one program to bite. The fix is a schema change plus a unique-index migration on the busiest write path in the app, which wants its own reviewed diff rather than riding along with a docs pass.
- **Unblocked by:** Nothing blocking. Add `program_exercise_id` (and ideally `program_set_id`) to `workout_sets`, written at log time, and move the unique index to `(session_id, program_exercise_id, set_number)`, keeping `exercise_id` for cross-program reads like PR detection. Backfill by joining on `(program_id, exercise_id, set_number)`, accepting ambiguity for sessions that already collided — that data is already lost. The progression revamp needs this same change for session-scoped clearance, so doing it once serves both; if that plan proceeds, do it there as `P-2` and delete this entry.
- **Touchpoints:** `src/db/schema/workout-sets.ts` (the `uniq_wsets_session_exercise_set` index), `src/lib/actions/workout-sets.ts` (`logWorkoutSet` 195-250, `unlogWorkoutSet`), `src/lib/actions/programs.ts` (`addExerciseToProgram`, which has no duplicate guard), `drizzle/`.
