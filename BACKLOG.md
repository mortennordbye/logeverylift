...
- **Touchpoints:** `src/db/schema/workout-sets.ts` (the `uniq_wsets_session_exercise_set` index), `src/lib/actions/workout-sets.ts` (`logSet` 195-250, `unlogSet`), `src/lib/actions/programs.ts` (`addExerciseToProgram`, which has no duplicate guard), `drizzle/`.
