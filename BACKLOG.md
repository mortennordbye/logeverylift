...
- **What:** Per-set RIR is now *logged* and feeds progression/adaptation (Jun 2026), but programs can't *prescribe* a target RIR. A full version adds a `targetRir` column to `program_sets`, sets it per phase (a phase-aware recipe returning a target RIR that ramps base→peak), shows "target RIR N" on each set like the existing target-HR-zone display, and feeds RIR into the LLM program-generation prompt (`ai-prompt.ts`) so generated programs can specify intended effort.
...
- **Unblocked by:** Wanting programs/AI to *target* an effort level, not just record it. Add `program_sets.targetRir` (+ migration), thread it through `addProgramSetSchema`/`updateProgramSetSchema` and `SetEditView` (program-edit mode), decide where a prescribed RIR is written now that phase re-prescription is gone (see "Strength periodization: considered, not implemented" in `docs/specs/cycle-periodization.md`), render it on the set row, and add it to the AI prompt/output schema.
- **Touchpoints:** `src/db/schema/programs.ts` (`program_sets`), `src/lib/validators/workout.ts`, `src/components/features/SetEditView.tsx`, `src/lib/actions/training-cycles.ts` (`syncPeriodizedTargets`), `src/lib/utils/ai-prompt.ts`, `src/lib/actions/ai-generate.ts`.
...
- **Why deferred:** Marginal CPU win. The UI already settles the chips to a tick for completed sets rather than acting on them (`WorkoutSetsList.tsx:1415`). Adds a DB query for negligible benefit.
- **Unblocked by:** Profiling that shows suggestion compute is a real bottleneck (unlikely with current dataset sizes).
- **Touchpoints:** `src/lib/actions/workout-sets.ts` (`getProgressiveSuggestions`).
...
### page-transition e2e hangs on a bottom-sheet backdrop
- **What:** `e2e/page-transition.spec.ts` "a push into a deeper route animates" hangs past its own 120s timeout: a backdrop (`fixed inset-0 z-50 flex items-end transition-all duration-300 opacity-100` — the readiness sheet) intercepts the click on the exercise link. Fails identically on `cd3e8f1`, so it is not from the progression rebuild. Never run before 2026-08-29.
- **Why deferred:** It ran under chromium rather than webkit (no webkit binary would download), and this spec is specifically about transition animation, which is the one area where that substitution is least trustworthy. Worth re-checking under webkit before spending time on it.
- **Unblocked by:** A working webkit install. If it still fails there, `dismissReadinessSheet` waits for the Skip button to be hidden but not for the backdrop's 300ms exit transition to finish, which is the obvious first suspect.
- **Touchpoints:** `e2e/helpers.ts` (`dismissReadinessSheet`, `openWorkout`), `src/components/ui/BottomSheet.tsx`, `e2e/page-transition.spec.ts`.
...
- **Why deferred:** Allowing a package to build decides which packages may run arbitrary code at install time, and it changes what actually gets built in the image. That is a supply-chain decision, not a build fix, and it wants to land on its own rather than folded into an unrelated release. Until then every entry under `allowBuilds` in `pnpm-workspace.yaml` is `false` (#43). pnpm 11 had written them as unresolved placeholders, which failed its dependency check before any script ran, the pre-push `pnpm verify` included. `false` is what pnpm 10 already did, so nothing is built that was not built before, and local `pnpm` commands and the hook no longer need a flag or an environment variable.
- **Unblocked by:** Deciding which entries to flip to `true`, then verifying that image optimization still behaves — if `sharp` has never been built, the current production image is already running without it and allowing it would change that behaviour rather than preserve it. Check before assuming either way.
- **Touchpoints:** `Dockerfile` (four `npm install -g pnpm@10` lines), `.github/workflows/ci.yml:34-37`, `pnpm-workspace.yaml`.

### Two shapes left by migration 0051 read Custom in the progression sheet
- **What:** Two configurations nobody chose match no preset, so the sheet labels them Custom. (1) `reps` exercises with `progression_required_hits` null: the engine gates them on `REQUIRED_HITS` (2), which is what rep mode did before the rebuild, but the Rep ladder preset fixes the gate at 1. Prod had 14 such exercises on 2026-09-14. (2) Timed exercises that 0051 backfilled to `load` with `backoff`: the sheet snaps the advance to `duration` for display, the Duration preset wants `hold`, and the engine does not snap at all, so it judges them by reps a timed set never logs. A whole-database count of shape (2) was not taken; one user has four (Cycling, Hollow Hold, two Warm Ups). The effort-cap cause of the same symptom was fixed in #43.
- **Why deferred:** Both fixes change behaviour, not just the label. Gating (1) on one session makes those exercises progress twice as fast; moving (2) to `duration` starts suggesting longer holds, and moving it to `manual` stops suggesting anything. The Ironman programs that surfaced this were corrected by hand instead.
...
### The rep-target e2e case has never been executed
- **What:** `e2e/progression-settings.spec.ts` gained a section that taps the Target reps chips, asserts the rule sentence follows, and restores the original target in the teardown. It type-checks and lints, but no run has ever exercised it: the local WebKit binary is not installed (`browserType.launch: Executable doesn't exist at .../webkit-2272`), and `mobile` is the only non-setup project.
- **Why deferred:** The same missing binary already blocks the whole suite locally, so this is not specific to the new case. The behaviour it covers was verified by hand instead — an 8/6/6/6 Bench Press levelled to 8/8/8/8 in the database from one chip tap, with the sentence following it — and the pure halves (`uniformTargetReps`, the re-judge branch, the schema) are covered by Vitest.
- **Unblocked by:** `npx playwright install webkit`, then `npx playwright test e2e/progression-settings.spec.ts --project=mobile`. Watch the teardown in particular: it only restores a target the chip row actually offers, so a seed that ever uses a custom target would leave the shared program changed.
- **Touchpoints:** `e2e/progression-settings.spec.ts` (the rep-target section and its restore), `e2e/helpers.ts` (`tapAndSave`), `playwright.config.ts`.

### Playwright WebKit: a tap right after the readiness sheet closes was dropped, not seen on iOS
- **What:** Under Playwright's WebKit, a tap on an in-app `<Link>` shortly after the readiness sheet closed was accepted by the anchor but the navigation never committed (exercise row → set list, and set row → set editor). The retries in `openFirstExercise` / `openFirstSetEditor` exist for it. The cause first written down for it no longer matches the code: `confirmReadiness` does not call `router.refresh()`, and `setSessionReadiness` does not revalidate.
- **Why deferred:** Not reproduced on 2026-09-14 in iOS Simulator Safari (iPhone 17 Pro) against the dev server: Skip followed by an immediate exercise tap navigated, and Back, reopen, then an immediate set-row tap opened the editor. Simulator taps land a few hundred ms apart, so a narrower window is not ruled out, and there is nothing to fix without a failing case.
- **Unblocked by:** A report of a dead tap on a real iPhone, or a Playwright WebKit run once the binary downloads (it hangs locally). Capture a trace of the failing tap before changing any component.
- **Touchpoints:** `src/components/features/WorkoutSessionClient.tsx` (`showReadiness`), `src/components/features/ReadinessSheet.tsx`, `src/components/features/WorkoutExerciseList.tsx` (the row `<Link>`), `e2e/helpers.ts` (`openFirstExercise`, `openFirstSetEditor`).
