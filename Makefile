...
dev: ## Build the image and start the containers (health-checked)
	...
	if [ -z "$(SKIP_BUILD)" ]; then
	  CACHE=""
	  if [ -n "$(CLEAN)" ]; then CACHE="--no-cache"; fi
	  echo "Building stage [$(STAGE)]..."
	  docker build $$CACHE --target $(STAGE) -t $(IMAGE) .
	fi
...
clean: ## Remove containers, volumes, and images (DESTROYS the DB; CACHE=1 also prunes build cache)
	...
	for tag in dev prod latest; do docker rmi logeverylift-pwa:$$tag 2>/dev/null || true; done
