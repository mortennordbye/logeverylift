...
- **Auto-complete days.** A slot with `autoComplete` on is a day tracked outside the app (a run on a Garmin, say). `getActiveCycleForUser` writes a completed `workout_sessions` row with `source = 'auto'` and no sets for it, so it counts everywhere a completed session counts: missed detection, rotation position, adherence, streaks, weekly goal, history, calendar and the friends feed. Which days qualify is the pure `autoCompleteDue` (`cycle-position.ts`). In day-of-week mode that is today and the previous 7 days from the cycle start; in rotation mode it is today's slot only. A day is left alone if a completed session for that program already exists on it, or its date is in `dismissed_makeups`. "I skipped it" on the home card (`skipAutoCompletedDay`) deletes the auto row and writes that dismissal, so it is neither recreated nor flagged as missed. Finishing a real session for the same program and date (`completeWorkoutSession`) deletes the auto row, so the day is not counted twice. The partial unique index `uniq_ws_auto_day` keeps concurrent renders from inserting it twice. The triathlon generator turns the flag on for swim/bike/run-only days; it is toggled per day in the cycle editor, set by `"auto": true` on a slot in an imported cycle (`importCycle`), or by `autoComplete` on the MCP `edit_cycle_slot` tool.
...
| Day | Session |
|---|---|
| 1 Mon | **Workout A** — Squat & Horizontal (Front Squat, DB Bench, Pendlay Row, Bulgarian Split Squat, Seated Calf Raise, Pallof Hold) |
...
Strength is a **flat hypertrophy/maintenance block**: same target reps across working sets, no phase re-prescription (strength sets carry no `sessionRole` at all), `weightKg: 0` (athlete loads to the target reps). Every rep-based lift runs **Load, confirmed**: the `load` advance with a positive kg increment, the default two-session gate and no `targetRir` cap, so it adds weight after two sessions in a row at the same weight and reps. Per-exercise type is set by the generator. **Pallof Hold** is the one timed hold: 2 × 30 s with a 5 s `startDelaySeconds`, on the `duration` advance, so the hold gets longer on the same gate. It is a separate exercise from Pallof Press because `isTimed` lives on the shared row and other programs press Pallof Press for reps. Migration `0053` adds it to existing databases.
...
> This is the map. What the curve is *supposed* to do — and the divergences found against it — is specified rule-by-rule in [specs/cycle-periodization.md](specs/cycle-periodization.md) (`PZ-1`…`PZ-44`).
