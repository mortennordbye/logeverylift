...
> **Status:** built, and smoke-tested. All seven phases have shipped and nothing is pushed. The `CLAUDE.md` smoke pass has now been run end to end against a real database and found one bug, since fixed; the release notes are written ([`releases/progression-rebuild.md`](releases/progression-rebuild.md)). What remains is the Playwright e2e suite, confirming `HONEST_REPS_FROM` against the real release date, and the push itself. See §0 "Build state".
...
- **An estimated-1RM record needs a logged RIR of 3 or better**, and records of the two derived types set before `HONEST_REPS_FROM` are flagged `unverified` in the PR feed. Kept, not deleted or recomputed (`D-10`). **The cutover is a constant and needs checking at deploy** — late over-flags, early presents assumed numbers as measured ones.
...
- **The Playwright e2e suite has never run on any of these branches, and it is blocked on the machine rather than on the code.** The dev server is no longer the obstacle — that was fixed, and the smoke pass ran against it. The obstacle is that **no browser binary for `playwright@1.59.1` will download here**: the cache holds `chromium-1234` and `chromium_headless_shell-1234`, and 1.59.1 wants `-1217`. Three attempts stalled part-way and stayed there — webkit twice at 15 MB (leaving only `libwebrtc.dylib`), chromium at 448 KB (leaving an app bundle with no framework binary). It is not the cache lock, which an earlier note blamed and which was cleared; the downloads themselves do not complete. Resolve from a network that can reach Playwright's CDN, then run `npx playwright install`. Phases 0, 3, 2 and 5 all touched the set list; phase 2 added `e2e/miss-sheet.spec.ts` and phase 5 rewrote `e2e/progression-settings.spec.ts` outright, and neither has run.

  Worth knowing before it does: the specs drive a **shared** program on the test account, and `progression-settings.spec.ts` restores what it changes at the end. A run that dies part-way leaves that exercise on whatever it last wrote.
- ~~**The `CLAUDE.md` smoke pass is owed for the whole rebuild.**~~ **Run 2026-08-29 against the dev database, 8 of 9 steps; step 5 (timed-set countdown) was skipped because no seeded program carries a timed set.** It found one bug — the exercise-level apply-all chip hardcoded an upward arrow, so a re-approach or a deload read as an increase — which no unit test covers, because none of them renders an arrow. The paragraph below is kept because the pass is owed again for anything that touches these screens. It needs `make dev` running, which is the owner's call to start. **Phase 5 is the one that most wants it**: it rewrote the progression sheet outright, added an inline effort prompt to the set list, and its query change has never run against a real database. Phase 3's history query is in the same position — no unit test executes SQL, and its rendered statement was checked by hand through `toSQL()`, which proves it parses, not that it returns the right rows. Phase 6 touches the PR feed, which step 9 of the pass covers.
...
- **`HONEST_REPS_FROM` must be checked against the actual release date** (`pr-provenance.ts`). It decides which personal records are shown as set before reps were logged individually, and there is no column that can answer that retroactively. Late over-flags a few genuine records, which understates the app's confidence; early presents assumed numbers as measured ones, which is the thing `D-10` exists to prevent. Set it late rather than early if the date is uncertain.
