...
> **Status:** in build. Phases 0, 1, 3, 2, 4 and 5 shipped; phase 6 is next and is the last. See §0 "Build state" — it is the one place that says where the work stands, and every phase updates it before finishing.
...
| **6** | The rest | **Next.** Not started. It is the last phase. |

**Branches.** `main` → `fix/progression-never-lowers-plan` (the audit, one predating bug fix, and this plan; 20 commits, **not pushed**) → `phase-0/progression-prerequisites` (5 commits, **not pushed**) → `phase-1/honest-effort` (7 commits, **not pushed**) → `phase-3/session-windows-and-scope` (4 commits, **not pushed**) → `phase-2/capture` (4 commits, **not pushed**) → `phase-4/rep-ranges` (5 commits, **not pushed**) → `phase-5/axes-and-presets` (6 commits, **not pushed**). One phase per branch, each branched off the last. Nothing has been pushed yet, so nothing is deployed and no migration has run outside a throwaway container.
...
*Still true, and still wrong — this is phase 6, not an oversight:*

- Staleness (`A10`), the increment ladder's ordering and loadable granularity (`A3`), the PR effort gate and `D-10`'s unverified flag (`A11`), the generated-plan prompt (`SI-D8`, still describing the retired modes), and `B-23`.
...
**Phase 6: the rest.** Increment ladder ordering **and equipment-loadable granularity** (`A3` has both halves), staleness (`A10`, with `E-19`'s frequency guard), PR effort gate and the `D-10` unverified flag (`A11`), generated-plan prompt (`SI-D8`), `B-23`.
...
`A1` (**closed**, phases 1 and 2), `A2` (**closed**, phase 5), `A3` (**both** halves, phase 6), `A4` (**closed**, phase 4), `A5` (**closed**, phase 5 — the fix is axis 6, not phase 3), `A6` (**closed**, phase 3 — the rule is section 7 step 2, not a phase-6 bullet), `A7` (**closed**, phase 5 with the readiness axis), `A8` (**closed**, phase 3), `A9` (**closed**, phase 5, with all three artefacts built), `A10` (phase 6), `A11` (phases 1 and 6).
...
**Smart-incrementation spec divergences:** `SI-D1` (**closed**, phase 5 — the shown 1RM now needs logged effort), `SI-D2` (**closed**, phase 5 — `D-4` deleted the field it leaked through), `SI-D3` and `SI-D4` (**closed**, phase 5 — the axes made both explicit settings), `SI-D5` (bodyweight never progresses — **still open**: the **Rep ladder** preset makes the setting reachable, which is not the same as fixing `overload_increment_reps` defaulting to 0, and that is `E-17`'s open half), `SI-D6` (**closed**, phase 3), `SI-D8` (generated plans, phase 6).
