...
- **Migrations `0046` to `0051` have not run against a real database.** They are generated, committed and reviewed; nothing has been applied outside a throwaway container. `0048` is a single `DROP NOT NULL`; `0049` is a single `ADD COLUMN … NOT NULL DEFAULT 'all'` whose default *is* the backfill; `0050` adds two nullable columns and backfills nothing; `0051` adds six columns and runs two `UPDATE`s over `program_exercises` — the first of these whose backfill reads a column rather than writing a constant.
