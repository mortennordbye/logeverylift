...
- **A settings change no longer re-judges history.** `progression_config_at` is stamped by every write that changes a judging rule, and the window drops sessions logged before it (`E-13`, which had no owner). Increments and the plan opt-in deliberately do not stamp it.
