...
- **The Playwright e2e suite has never run on any of these branches.** The webkit binary is **installed now** (phase 5), so the remaining blocker is only a dev server at `localhost:3000`, which is the owner's to start. Phases 0, 3, 2 and 5 all touched the set list; phase 2 added `e2e/miss-sheet.spec.ts` and phase 5 rewrote `e2e/progression-settings.spec.ts` outright, and neither has run.
