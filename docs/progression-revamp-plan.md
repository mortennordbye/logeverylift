...
1. ~~**Set `HONEST_REPS_FROM` to the real release date.**~~ **Done 2026-08-31: it is `2026-09-01`** (`src/lib/utils/pr-provenance.ts`), the day after the intended deploy, so a deploy landing at any hour of the 31st still has every record the old code produced behind the cutover. It decides which personal records are shown as "set before reps were logged individually", and no column can answer that retroactively — before the release, a tap wrote the target back as the result, so those records are derived from reps nobody counted. **If the deploy slips past 2026-09-01, move the constant forward.** Late over-flags a few genuine records and understates the app's confidence; early presents assumed numbers as measured ones, which is the thing `D-10` exists to prevent.
...
- **The engine reads axes, not a mode.** `program_exercises` carries `progression_advance`, `progression_regress`, `progression_backoff_pct`, `progression_backoff_after`, `progression_readiness` and `progression_config_at` (migration `0051`), backfilled from the mode per section 10. `progressionMode` still exists and is still *written* — sharing and export both emit it, and a client cached before this release has nothing else to read — but **nothing reads it**, and `E-12` says it goes a release later. Do not add a read.
- **`advance` has a `manual` value that section 5 does not list, and it has to.** Manual and Off differ only in whether the last session's numbers are shown, and no other column carries that difference. Recorded here because it is a deviation from the plan, not an oversight in it.
- **An effort cap gates clearing, and the set the scope names decides it.** `getProgressiveSuggestions` selects `target_rir` at last (`B-22`), resolves it against the scope exactly as `D-8` describes, and hands `buildSuggestion` one resolved `effortCap`. Uncapped exercises clear on the target alone. Effort below the cap is a **miss**; effort unlogged is `held-unknown`, which is emitted for the first time.
- **`smart` is gone.** The mode, its rep-cut field and `estimateRepsAt` are all deleted (`D-4`), and `estimated1RM` now needs a logged RIR of 3 or better, which closes `SI-D1`. `smart` exercises migrated to plain `load`; none became double progression.
- **Duration and distance advance from the target, and an anchored set never advances at all.** Both halves of `A5`. A set carrying `peak_duration_seconds` or `peak_distance_meters` gets `held-anchored` — the cycle owns those columns, and a progression write there is overwritten at best.
- **Three new reason codes, all walked through the eight sites in `B-2`:** `held-unknown` (now emitted), `held-no-increment` (`E-4`'s message, finally built) and `held-anchored`. `re-approach` is still owed by phase 6, and owes the same walk. **The family is still named `progressed*`** — two sites match on the prefix and a rename breaks them with no type error.
- **A settings change no longer re-judges history.** `progression_config_at` is stamped by every write that changes a judging rule (`E-13`, which had no owner), and a session older than it is **unknown**, not dropped. Inert, not absent: dropping them empties the window, and an empty window returns no suggestion at all — so changing a setting would delete the chip, the dots and the "Last: 80kg" line as a side effect of asking a question about them. Increments and the plan opt-in deliberately do not stamp it.
- **The preset is derived, never stored.** `progression-presets.ts` matches live axis values against section 3's table; anything unmatched is **Custom**. A stored preset would be display-only state the engine does not read — which is exactly how `target_rir` sat dead for a year.
- **The sheet has three layers and the sentence is layer 2, not a footnote.** It quotes the cap, the back-off and the readiness rule. The rep target, the rep range and the effort cap are written across every working set of the slot from here; `SetEditView` still edits all three one set at a time, and during a workout it cannot edit the target at all (`SI-9a`). The sentence quotes a rep target only when every working set agrees on one.
- **An exercise with a cap gets one effort prompt, after its last working set.** Section 6's change 3, and it writes through the queue-backed writer like every other log.
- **The plan's §10 was wrong about one thing and the migration says so.** `reps` exercises back off in the old engine and the **Rep ladder** preset holds, so migration `0051` moves them to `hold`. That is the only place it changes behaviour rather than recording it, and it is owed a line in the release note.

*From phase 6:*

- **The increment ladder runs load-zone first, experience second, equipment last.** Experience level used to return before the zone table was reached, so anyone with a profile never got load scaling; it is a multiplier now (beginner ×2, advanced ×0.5, intermediate unchanged). The result snaps to `loadableStepKg` — barbell 2.5, dumbbell 2, machine and cable 1.25, kettlebell 4. **An exercise with no equipment recorded is deliberately not snapped**; most of the library is in that state and a default step would coarsen all of them on no evidence. Do not give `loadableStepKg` a fallback.
- **A missing rep increment means one rep.** `overload_increment_reps` still defaults to 0 — that is what "unset" looks like — but the engine reads it as one rep rather than as "no progression", which is what `SI-D5` was actually asking for. Bodyweight exercises progress out of the box now. The rep-range ceiling still caps the climb, including on the bodyweight fallback.
- **Staleness is `re-approach`, and it is asked before the back-off.** 21 days, or 2.5x the exercise's own median gap when that is longer (`E-19`). A layoff and a stall want opposite responses, so the order is load-bearing. `re-approach` is the last new reason code; it was walked through all eight `B-2` sites, is exempt from the plan floor like a back-off (`E-2`), and is excluded from the stagnating headline. **`buildSuggestion` now takes `today` as an argument** rather than reading the clock, so it stays pure and a fixture can sit three months after its own history — do not replace that with `new Date()`.
- **An estimated-1RM record needs a logged RIR of 3 or better**, and records of the two derived types set before `HONEST_REPS_FROM` are flagged `unverified` in the PR feed. Kept, not deleted or recomputed (`D-10`). **The cutover is a constant**, set to `2026-09-01` for this release and still owed a check if the deploy slips past that date — late over-flags, early presents assumed numbers as measured ones.
- **The AI plan prompt describes the axes.** All six advance values, the gate, a worked rep-range exercise, and what an effort cap costs before it is offered. It still does not offer `progressionApplyToPlan`, deliberately: the ratchet rewrites a lifter's plan and a generator opting them into it is not a default anyone chose. The import writes the retired mode column from the axis, so an imported program that is later shared does not arrive describing a different scheme.

*Nothing is still wrong that a later phase was going to fix.* The open items are `SI-D7` (the inert global increment controls in Settings, a Settings question this plan does not touch) and the carry-forward below, none of which is code.

### Carry-forward: things every phase still owes

- ~~**The Playwright e2e suite has never run on any of these branches.**~~ **Run 2026-08-29: 7 of 10 passed.** No browser binary for `playwright@1.59.1` will download here — the cache holds `-1234` builds, 1.59.1 wants `-1217`, and every fetch stalls part-way — so it ran against the complete `chromium-1234` already in the cache, through an `executablePath` override. **That is not webkit**, which `devices["iPhone 14 Pro"]` selects and which a PWA targeting iOS actually ships to, so engine-specific behaviour is still untested. Everything else the specs assert is app behaviour, and that is now covered.

  Of the three failures, one was a defect in phase 5's own spec — an assertion matching text the preset list also contains — and is fixed. The other two are **pre-existing**, confirmed by re-running them against `cd3e8f1`, the phase 4 tip, where they fail identically. The check is sound: the `re-approach` chip phase 6 adds appears in the phase-6 failure context and not in the phase-4 one, so the dev server really was serving the older code.

- **`e2e/miss-sheet.spec.ts` fails, and it is catching a real bug rather than being flaky.** Phase 2 shipped the spec without ever running it. The spec logs a short set through the miss sheet correctly, then its cleanup taps the toggle to un-log and **nothing happens**: the set stays logged and its rest timer keeps counting, so `toggleSet`'s un-log branch never ran. The failure screenshot shows the set still ticked, "· 6 done", REST at 01:25. Reproduced on the phase 4 tip, so it predates every phase in this plan. Deliberately not fixed here — it is phase 2's territory, the cause is not obviously one line, and folding it into phase 6 would bury it. **It is the first thing to pick up after this plan lands.**

- **`e2e/page-transition.spec.ts` fails the same way on both commits**, hanging past its own 120s timeout while a bottom-sheet backdrop (`transition-all duration-300` — the readiness sheet, not one this plan touched) intercepts a click on the workout page. Never run before either. Unlike the miss sheet, chromium-instead-of-webkit is a plausible contributor here, since the spec is about transition animations.
- **Migrations `0046` to `0051` have not run against production data.** They are generated, committed, reviewed, and **all 52 apply cleanly to an empty Postgres 16 with no schema drift** (`drizzle-kit migrate` then `drizzle-kit check`, phase 5). `0048` is a single `DROP NOT NULL`; `0049` is a single `ADD COLUMN … NOT NULL DEFAULT 'all'` whose default *is* the backfill; `0050` adds two nullable columns and backfills nothing. `0051` adds six columns and runs two `UPDATE`s over `program_exercises` — the first of these whose backfill reads a column rather than writing a constant, so it was **replayed over a fixture row carrying every legacy mode value including null**, and produced section 10's table exactly. What is still untested is volume and real data, not the statements.
- ~~**`HONEST_REPS_FROM` must be checked against the actual release date.**~~ **Set 2026-08-31 to `2026-09-01`** (`pr-provenance.ts`), the day after the intended deploy. What is still owed is one check at deploy: if it lands after 2026-09-01, move the constant forward, or records produced by the old code in the gap are presented as measured ones.
- **Six release notes are owed at the end, and none of them is a code change.** Phase 1 retired the RPE ladder, so exercises a lifter grinds will start bumping. Phase 3's `D-3` now holds exercises that were quietly banking per-set counts, and the same phase stopped dropping Tired sessions, so a lifter who reports fatigue will see their numbers move again instead of freezing. Phase 2 made the third due (`B-4`): volume has always been planned volume relabelled as achieved, so every volume number in the app falls for anyone who misses a rep, starting the day this ships. Phase 5 adds two. A weighted rep-ladder exercise stops backing off, because migration `0051` moves `reps` to regress `hold`. And every plan the triathlon generator has produced starts gating on the RIR caps it has always written, because `D-1` makes them load-bearing for the first time — intended, but nobody asked for it at generation time (`B-9`). Phase 6 adds the sixth, and it is the broadest of all: **every suggested increment changes size**. The ladder reordering moves the number for anyone with an experience level on their profile, and the granularity rule moves it again for anyone whose exercises record their equipment. Both are corrections, both are visible on the first load after deploy, and neither is something a lifter did.
...
| `progression_advance` | **new**, text | **Done, phase 5** (migration `0051`), `not null default 'manual'`, backfilled from the mode per section 10. `load` / `reps` / `double` / `duration` / `distance` / `none`, **and `manual`** — this list was one value short. Manual and Off differ only in whether the last session's numbers are shown, and no other column carries that. |
| `progression_regress` | **new**, text | **Done, phase 5.** `hold` / `backoff`. |
| `progression_backoff_pct` | **new**, int | **Done, phase 5.** Only read when regress is `backoff`. |
| `progression_backoff_after` | **new**, int | **Done, phase 5.** Consecutive non-clearing sessions before backing off. |
| `progression_readiness` | **new**, text | **Done, phase 5.** `ignore` / `hold` / `reduce`. |
| `progression_config_at` | **new**, timestamp | **Done, phase 5** (migration `0051`), nullable, and not in this table's original list. `E-13` asks for a config version and this is it: sessions logged before it drop out of the window, so a settings change is judged from the change forward. Stamped by every write that changes a judging rule; deliberately not by the increments, the plan opt-in, a rep target (`SI-9a`), or a write that changes nothing. |
...
**Phase 5: axes, presets, and the sheet. Built.** The remaining axis columns (migration `0051`), the preset table, the extended sentence, the three-layer sheet, `B-22`'s query change, and the effort cap it unblocked. `progression_mode` is unread and kept for a release (`E-12`); `smart` and its rep-cut field are deleted (`D-4`). Also built here because they were the same read path: `A5`'s two halves, the regress and readiness axes, `E-4`'s message, `E-13`'s config stamp, and section 6's change 3.

Shipped on branch `phase-5/axes-and-presets`. Section 5's migrations are complete. `A9`'s three artefacts are all built — the **Inputs — provenance** section, the **rate rule** as `SI-41`, and steps 4a and 4b in `.claude/skills/feature-spec/SKILL.md` — so `A9` closes; a rewrite without them would have reproduced the blind spot it exists to name. Also closed `A2`, `A5`, `D-1`, `D-4`, `D-8`'s effort half, `E-4`, `E-7`, `E-13`, `E-18`, `SI-D1`, `SI-D2`, `SI-D3`, `SI-D4`, and consumers `B-7` to `B-10`, `B-12`, `B-13`, `B-19`, `B-20` and `B-22`.

**It corrected §10**, which called the `reps` migration "no change". A rep ladder backs off in the old engine and the **Rep ladder** preset holds, so `0051` moves them to `hold`; that is the one place the migration changes behaviour rather than recording it. **It also added a value section 5 does not list:** `advance` carries `manual`, because Manual and Off differ only in whether the last session's numbers are shown and no other column says which.

**Phase 6: the rest. Built.** Increment ladder ordering **and** equipment-loadable granularity (`A3`, both halves), staleness with `E-19`'s frequency guard (`A10`), the PR effort gate and `D-10`'s unverified flag (`A11`), the generated-plan prompt (`SI-D8`, `B-23`), and `E-17`'s open half — a missing rep increment now means one rep, which closes `SI-D5`.

Shipped on branch `phase-6/the-rest`, no migration: every change is behavioural or a constant, and `D-10`'s flag is derived from a date rather than a column, exactly as the decision proposed. This is the last phase; §13 below is now fully closed apart from `SI-D7`.

`A6` (Tired) is **not** here: its rule is section 7 step 2 and landed in phase 3 with the query rewrite. `A5` landed in phase 5 with axis 6, along with its anchored-set exclusion (section 9). `A7` (readiness `reduce`) landed in phase 5 too — it is the readiness axis, and building the axis without its third value would have shipped a picker with a dead option.

Staleness adds `re-approach`, which is the last new reason code and owes the same eight-site walk as `B-2` describes. `E-2` exempts it from the plan floor, exactly as `backoff` is.
...
**All eleven are closed.** `A1` (phases 1 and 2), `A2` (phase 5), `A3` (**both** halves, phase 6), `A4` (phase 4), `A5` (phase 5 — the fix is axis 6, not phase 3), `A6` (phase 3 — the rule is section 7 step 2, not a phase-6 bullet), `A7` (phase 5, with the readiness axis), `A8` (phase 3), `A9` (phase 5, with all three artefacts built), `A10` (phase 6), `A11` (phases 1 and 6). Their `BACKLOG.md` entries are deleted.
...
**Smart-incrementation spec divergences:** `SI-D1` (**closed**, phase 5 — the shown 1RM now needs logged effort), `SI-D2` (**closed**, phase 5 — `D-4` deleted the field it leaked through), `SI-D3` and `SI-D4` (**closed**, phase 5 — the axes made both explicit settings), `SI-D5` (**closed**, phase 6 — a missing rep increment means one rep, which is `E-17`'s other half; the column still defaults to 0 because that is what "unset" looks like), `SI-D6` (**closed**, phase 3), `SI-D8` (**closed**, phase 6).

**Cycle-periodization spec divergence:** `PZ-D3` (strength phase has no producer), closed by decision `D-6`.

**Stays open:** `SI-D7`, the inert global increment controls in Settings, and it is the only thing that does. It is a Settings question, not a progression-engine one, and this plan deliberately does not touch it — the decision it needs is what the control should *mean* (a seed for new exercises, a fallback in the ladder, or nothing), which is a product call rather than an engine one.
...
**E-2. Built in phase 6. `re-approach` may lower the plan.** It is a back-off by another name (`D-5`), so it is exempt from the floor exactly as `backoff` is.
...
**E-4. Built: the engine half in phase 4, the message and the sheet's guard in phase 5. `advance: double` requires a positive load increment.** At `rep_range_max` with no load to add, the reset cannot happen and the exercise would sit at the top of its range forever. The sheet must not offer `double` without an increment, and the engine holds with `held-no-increment` — `held-unknown`'s sibling, reading "set a weight increment to progress" — rather than silently climbing past the range the user configured. Bodyweight work uses `advance: reps` and no range, which is what `SI-D5` was really asking for.
...
**E-7. Built in phase 5. Readiness `reduce` reuses the `backoff` reason code** with `readinessModulated: true`, rather than adding a tenth code. Both consumers already branch on `backoff`, and the distinction is a display concern (the chip says why), not a different write.
...
**E-13. Built in phase 5. Changing an exercise's configuration re-judges history retroactively.** Switch preset, gate, scope, or fixed-to-range, and the existing 5-session window is re-evaluated under the new rule: sessions that cleared yesterday can un-clear today. `E-9` protects against plan *shape* changes but nothing protects against *rule* changes. Stamp a config version on the exercise and count only sessions logged since the last change. Anything else means the dot count moves when the lifter touches a setting, which reads as a bug. *Built as `program_exercises.progression_config_at`, stamped by every write that changes a judging rule — the axes, the rep range and the effort cap — and by nothing else. Narrowed twice since: a write that changes nothing does not stamp, and a rep target is re-judged from the log rather than discarded (`SI-9a`), so it does not either.*
...
**E-17. Fully built: the ceiling in phase 4, the zero-increment half in phase 6. `advance: reps` has no ceiling.** `E-4` guards `double` against a missing load increment; the mirror case is a rep ladder with nowhere to stop, which climbs 8, 9, 10 … 40 forever. This is also the real content of `SI-D5`: the rep fallback needs `overload_increment_reps > 0` and that column **defaults to 0** (`schema/programs.ts:61`), so bodyweight exercises hold forever out of the box. Section 13 claimed `advance: reps` closes `SI-D5`; it does not unless the zero-increment default is fixed too. Treat a missing rep increment as 1 in the engine (no migration) and give `advance: reps` an optional `rep_range_max` stop.

**E-18. Built in phase 5. A back-off has no floor.** Repeated 10% cuts walk a lift below an empty bar, and at `baseWeight = 0` (bodyweight) both the back-off and the `re-approach` compute to 0. Floor the result at one increment, and make back-off a no-op at zero load.

**E-19. Built in phase 6, with staleness itself. Staleness is absolute and will misfire on low-frequency work.** `D-5`'s 21 days is measured from the last session, so an exercise deliberately trained on a three-week rotation sits permanently at `re-approach` and is offered -10% every session. Consider `max(21 days, 2.5x the median inter-session interval for that exercise)`.

**E-12. Rollback. Phase 5 holds to it: `progression_mode` is unread but kept and kept written, and both wire formats still emit it.** Every migration is additive (new nullable columns, one column made nullable) except `P-2`'s unique index replacement, which needs a tested down-path. `D-3`'s scope migration changes behaviour rather than shape, so the previous `progression_mode` values must survive it. **That contradicts section 5's "retired after migration".** Resolve it by keeping the column, stopping all reads of it in phase 5, and dropping it a release later. The same staged retirement applies to the wire formats in `B-7`, `B-13` and `B-15`.
...
**B-8. The MCP tool exposed the mode as an enum. Done in phase 5.** `mcp/tools/programs.ts` validated `progressionMode: z.enum(PROGRESSION_MODES)` and defaulted it. **Done in phase 5:** the tool takes the axes, still accepts the old values and maps them, and the description says which to prefer.

**B-9. The triathlon generator sets schemes and RIR caps directly. Done in phase 5**, and it owes a release note — see §0's carry-forward. `triathlon-plan.ts` writes `progressionMode` per exercise and `targetRir` per set, and its comment explains it deliberately avoids `smart`. Phase 5 must move it to presets. With `D-1` and `D-8` live, its existing `targetRir` caps **become load-bearing for the first time**: plans it generates will start gating progression on effort. That is the intended behaviour, but it is a behaviour change to generated plans that nobody asked for at generation time.

**B-10. The e2e progression spec asserted the old sheet. Done in phase 5:** it drives the preset, the sentence, the gate and the scope, and asserts the relabel to Custom. `e2e/progression-settings.spec.ts` drives the "Sessions at target" gate group and asserts the rule sentence quotes the live gate. Phase 5's three-layer sheet rewrites both. *Required:* update it in the same change, and extend it to cover the preset picker, since it is the only end-to-end coverage progression has and it doubles as the missing-migration canary.
...
**B-12. The demo user shares tables with real users. Done in phase 5:** the seeders now carry a fixed-target exercise, a rep-range one, one capped and one not. Anything seeded is visible in the demo account, and the demo seeder is `admin.ts:420` (`seedDemoDataForUser`), not anything under `scripts/`. Seed data should exercise the new schemes (a fixed-target exercise, a rep-range exercise, one capped, one not) so the presets are demonstrable, not just implemented.
...
**B-19. Two more places gated the mode enum. Done in phase 5.** `importProgramSchema`'s mode enum in `validators/workout.ts` (which gates the import `B-13` describes) and a `VALID_PROGRESSION_MODES` constant in `programs.ts` backing the mode-setting Server Action. Neither was covered by `B-7`, `B-8` or `B-13`. **Done in phase 5:** the import enum still accepts every legacy value and maps it, and the mode action is replaced by one that writes the axes.

**B-20. The mode union lived in the component. Done in phase 5** — it is gone, and the component holds an `Axes` shape built from the engine's exported unions. `WorkoutSetsClient.tsx:39`, `:53`, `:357`, `:585` hold the type, the picker list, the badge label and a layout conditional. Phase 5 rewrites this file anyway, but section 8 never says the type is defined here rather than in `types/`.
...
**B-22. `getProgressiveSuggestions` did not select `target_rir`. Done in phase 5**, first thing, as the brief required. The query selected the set type, the mode, the gate and the rest, but not `targetRir`, so `D-1` had nothing to gate on. **The query change was a phase 5 prerequisite and was missing from the plan entirely.** It now also selects `rir` on the logged rows and the two cycle anchors, and resolves the cap against the scope before handing it to the engine.
